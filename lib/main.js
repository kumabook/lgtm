...
var fetchHoresase = function(images, requests) {
  var deferred = promise.defer();
  promise = fetchJSONAsync(horesaseApiUrl, requests).then(function(boyes) {
    for (var i = 0; i < perPage; i++) {
      var boyId = Math.floor(Math.random() * boyes.length) + 1;
      var boy = boyes[boyId];
      images.push({
        id: 'boy' + boy.id,
        imageUrl: boy.image,
        markdown: generateMarkdown(boy.body,
                                   boy.image,
                                   horesaseUrl + '?eid=' + boy.eid)
      });
    }
    deferred.resolve(boyes);
  }, function(event) {
    deferred.reject(event);
  });
  return deferred.promise;
